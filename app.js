...
function moduleMatches(module) {
  const haystack = [module.title, module.theme, ...module.focus, ...module.difficulty, ...module.grammar, ...module.words.flat(), ...module.exercises].join(' ').toLowerCase();
...
function renderNav(visibleModules) {
  moduleNav.innerHTML = visibleModules.map(module => `
    <button class="nav-btn ${module.id === state.selectedId ? 'active' : ''}" type="button" data-select="${module.id}">
      ${escapeHtml(module.title)}<br><small>${escapeHtml(module.theme)}</small>
    </button>
  `).join('');
}
  ...
  return `
    <article>
      ...
      <div class="section-grid">
        ...
        <section class="panel"><h4>语法点</h4>${list(module.grammar)}</section>
      </div>
      <section class="panel">
        <h4>核心单词：中英释义 + 音标 + 例句</h4>
